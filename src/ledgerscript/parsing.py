...
ArithmeticOperators: t.TypeAlias = t.Literal[TokenKind.PLUS] | t.Literal[TokenKind.MINUS] | t.Literal[TokenKind.MUL] | t.Literal[TokenKind.DIV]
...
def make_tokenizer[T](rules: dict[str, T], regex_prefix: str = r"\s*(", regex_postfix: str = r")\s*") -> t.Callable[[str], list[Token[T]]]:

    compiled_rules = list(map(lambda x: (re.compile(f"{regex_prefix}{x[0]}{regex_postfix}"), x[1]), rules.items()))
    def tokenize(source: str) -> list[Token[T]]:
        ...
        while (match_and_kind := next(
            filter(lambda x: x[0] is not None,
                   map(lambda x: (x[0].match(source), x[1]),
                       compiled_rules)), None)):
    ...
    for token in tokens:
        match token.kind:
            ...
            case TokenKind.PLUS | TokenKind.MINUS | TokenKind.MUL | TokenKind.DIV:
                while op_stack and (tos := op_stack[-1]) and tos != TokenKind.LPAREN and op_precedence[token.kind] < op_precedence[tos]:
                    op_stack.pop()
...
def produce(operator: ArithmeticOperators, production_stack: list["Production"]):
    match operator:
        case TokenKind.PLUS:
            return BinOp(op.add, right=production_stack.pop(), left=production_stack.pop())
        case TokenKind.MINUS:
            return BinOp(op.sub, right=production_stack.pop(), left=production_stack.pop())
        case TokenKind.MUL:
            return BinOp(op.mul, right=production_stack.pop(), left=production_stack.pop())
        case TokenKind.DIV:
            return BinOp(op.truediv, right=production_stack.pop(), left=production_stack.pop())
