...
def cli(argv: list[str], stdin: t.TextIO = sys.stdin):
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        nargs="+",
        help="Input file(s) to process. If none is specified, standard input is used.",
    )
    parser.add_argument(
        "--target",
        "-t",
        default="./target",
        help="The output directory for the compiled files if input files are specified. If no input files are specified, specifying this as an argument has no effect.",
    )
    ...
    if files:
        for compiled_file in compile(
            map(lambda x: SourceFile(content=open(x).read(), path=x), files)
        ):
