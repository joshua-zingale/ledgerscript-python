...
class RedifinitionError(RuntimeError):
    def __init__(self, redefined_names: t.Sequence[t.Sequence["Definition"]]) -> None:
        ...
        super().__init__(f"The following name{"s" if len(redefined_names) > 1 else ""} have multiple definitions: {",".join(map(lambda x: x[0].name, redefined_names))}")
...
class CircularDefinitionError(RuntimeError):
    def __init__(self, affected_definitions: t.Collection[str]) -> None:
        ...
        super().__init__(f"The following names are involved in a circular definition: {",".join(map(lambda x: x, affected_definitions))}")
...
class MissingDefinitionError(RuntimeError):
    def __init__(self, undefined_names: t.Collection[str]) -> None:
        ...
        super().__init__(f"The following names are undefined: {",".join(map(lambda x: x, undefined_names))}")
...
def resolve_definitions(definitions: list["Definition"]) -> dict[str, float]:
    ...
    if len(names) < len(definitions):
        ...
        for definition in definitions:
            name_to_definitions[definition.name] = name_to_definitions.get(definition.name, []) + [definition]
        raise RedifinitionError(list(map(lambda x: x[1], filter(lambda x: len(x[1]) > 1, name_to_definitions.items()))))
    ...
    while len(names) > len(defined_names):
        ...
        for undefined_name in filter(lambda n: not name_to_definition[n].dependencies - defined_names, names - defined_names):
            namespace[undefined_name] = eval(name_to_definition[undefined_name].production, namespace)
